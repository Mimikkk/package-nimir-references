...
describe('References - ReferenceSource', () => {
  describe('fetchAll mode', () => {
    ...
    it('returns null for IDs not in the dataset', async () => {
      const store = ReferenceSource.from<Entity>({ list: async () => [a1] });
    ...
    it('uses custom key function', async () => {
      ...
      const store = ReferenceSource.from<(typeof items)[0]>({ list: async () => items, keyBy: i => i.code });
  ...
  describe('fetch mode', () => {
    it('fetches and caches items by ID', async () => {
      ...
      const store = ReferenceSource.from<Entity>({ batch: fetch });
    ...
    it('negative cache entry expires after negativeTtl', async () => {
      ...
      const fetch = vi.fn(async () => []);
      const store = ReferenceSource.from<Entity>({ batch: fetch, ttlMs: 1 });
    ...
    it('chunks requests by batchSize', async () => {
      ...
      const batchSize = 2;
      const store = ReferenceSource.from<Entity>({ batch: fetch, batchSize });
